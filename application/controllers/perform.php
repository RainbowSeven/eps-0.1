...
class Perform extends CI_Controller {
  ...
  public function checkin() {
    if ( $this->session->userdata( 'login' ) ) {
      if ( ! $this->input->post( 'action_btn' ) ) //check in form not submitted
        {
  ...
  public function checkout() {
    ...
    if ( ! $this->input->post( 'action_btn' ) ) //check out form not submitted
      {
      $this->load->model( 'project_model' );
      $project_listing = $this->project_model->listing( $this->session->userdata( 'deptid' ) );
      $this->load->view( 'check/out_form', array( 'project_listing' => $project_listing,
          'clockout_option' => $this->department_model->has_clockout_option() ) );
    } else {
  ...
  public function view_employees( $deptid = '' ) {
    if ( $deptid == '' ) {
      if ( ! $this->input->post( 'action_btn' ) ) $this->load->view( 'select_dept',
          array( 'departments' => $this->department_model->get_department_list(), 'url' =>
            'perform/view_employees' ) );
  ...
  public function search() {
    if ( ! $this->input->post( 'action_btn' ) ) $this->load->view( 'search_form' );
    else  $this->load->view( 'search_result', array( 'needle' => $this->input->post
          ( 'needle' ), 'results' => $this->employee_model->search( $this->input->post( 'needle' ) ) ) );
  }
  public function org_chart() {

  }
  public function view_checked_in() {
    $this->load->view( 'employee_checked_in', array( 'checkers' => $this->
        employee_model->get_employee_checkedin(), 'department' => $this->
        department_model->get_department_name() ) );
  }
  public function view_all_checked_in() {
    $this->load->view( 'employee_checked_in', array( 'checkers' => $this->
        employee_model->get_employee_checkedin( 'all' ), 'department' =>
        'all departments' ) );
  }
  public function view_employee_info( $empid = '' ) {
    if ( $empid != '' ) {
      ...
      if ( $this->employee_model->has_manager( $empid ) ) {
        $data['manager'] = $this->employee_model->get_employee_name( $this->
          employee_model->has_manager( $empid ) );
      } else  $data['manager'] = 'No managers';
      $this->load->view( 'view_employee_detail', $data );
    }
  }
  public function view_timerecord() {
    $this->load->view( 'time_record', array( 'records' => $this->employee_model->
        get_time_records() ) );
  }
